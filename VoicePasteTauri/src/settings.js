(function () {
    ...
    const copy = {
        en: {
            settingsApp: "Settings", general: "General", models: "Models", remote: "Remote", advanced: "Advanced", permissions: "Permissions", appLanguage: "Application language",
            ...
            advancedTitle: "Advanced", warmupTitle: "Warm up remote server at speech start", warmupHint: "Sends an empty audio request. Useful for a custom Whisper server that sleeps between requests.", localCommand: "Parakeet command template", localCommandHint: "The command must write plain text to {output_path}; stdout also works.", permissionsTitle: "Permissions", microphone: "Microphone", accessibility: "Accessibility", granted: "Granted", notGranted: "Not granted", refreshPermissions: "Refresh status", openSystemSettings: "Open system settings", settingsFooter: "Changes are saved to your local VoicePaste configuration.",
            generalSection: "General", modelsSection: "Models", remoteSection: "Remote", advancedSection: "Advanced", permissionsSection: "Permissions", unavailable: "Unavailable"
        },
        ru: {
            settingsApp: "Настройки", general: "Общие", models: "Модели", remote: "Удалённый", advanced: "Расширенные", permissions: "Разрешения", appLanguage: "Язык приложения",
            ...
            advancedTitle: "Расширенные", warmupTitle: "Прогревать удалённый сервер в начале речи", warmupHint: "Отправляет пустой запрос аудио. Нужно для кастомного Whisper-сервера, который засыпает между запросами.", localCommand: "Команда Parakeet", localCommandHint: "Команда должна записать обычный текст в {output_path}; stdout тоже поддерживается.", permissionsTitle: "Разрешения", microphone: "Микрофон", accessibility: "Доступность", granted: "Разрешено", notGranted: "Не разрешено", refreshPermissions: "Обновить статус", openSystemSettings: "Открыть системные настройки", settingsFooter: "Изменения сохраняются в локальную конфигурацию VoicePaste.",
            generalSection: "Общие", modelsSection: "Модели", remoteSection: "Удалённый", advancedSection: "Расширенные", permissionsSection: "Разрешения", unavailable: "Недоступно"
        },
        zh: {
            settingsApp: "设置", general: "常规", models: "模型", remote: "远程", advanced: "高级", permissions: "权限", appLanguage: "应用语言",
            ...
            advancedTitle: "高级", warmupTitle: "开始讲话时预热远程服务器", warmupHint: "发送空音频请求。适用于请求之间会休眠的自定义 Whisper 服务器。", localCommand: "Parakeet 命令模板", localCommandHint: "命令必须将纯文本写入 {output_path}；也支持 stdout。", permissionsTitle: "权限", microphone: "麦克风", accessibility: "辅助功能", granted: "已允许", notGranted: "未允许", refreshPermissions: "刷新状态", openSystemSettings: "打开系统设置", settingsFooter: "更改保存到本地 VoicePaste 配置。",
            generalSection: "常规", modelsSection: "模型", remoteSection: "远程", advancedSection: "高级", permissionsSection: "权限", unavailable: "不可用"
        }
    };
    ...
    const sectionCopy = {
        general: ["generalSection", "generalSubtitle"], models: ["modelsSection", "modelsSubtitle"], remote: ["remoteSection", "remoteSubtitle"], advanced: ["advancedSection", "advancedSubtitle"], permissions: ["permissionsSection", "permissionsSubtitle"]
    };
    let language = "en";
    let state = null;
    ...
    function renderLocalStatus() {
        const local = state.local_model_status || {};
        const whisperReady = local.state === "ready" && state.local_model === "whisper-base";
        setText($("whisper-status"), whisperReady ? t("ready") : t("notDownloaded"));
        $("whisper-status").className = `badge ${whisperReady ? "good" : ""}`;
        setText($("whisper-path"), whisperReady ? `${local.path || ""} · ${formatBytes(local.bytes)}` : state.models_dir || "");
        const parakeetReady = state.local_model === "parakeet-v3" && local.state === "command_ready";
        setText($("parakeet-status"), parakeetReady ? t("commandReady") : t("commandMissing"));
        setText($("native-status"), state.native_available ? t("ready") : t("macOnly"));
        $("download-whisper").disabled = state.local_model === "whisper-base" && state.downloadInProgress;
    }
    ...
    async function init() {
        ...
        $("download-whisper").addEventListener("click", async () => { state.downloadInProgress = true; renderLocalStatus(); await invoke("download_local_model"); });
        ...
        await listen("local-model-progress", (event) => {
            const progress = event.payload || {};
            if (progress.state === "downloading") setText($("whisper-status"), `${t("downloading")} ${progress.total ? Math.round((progress.downloaded / progress.total) * 100) : ""}%`);
            if (progress.state === "ready") { state.downloadInProgress = false; load(); }
            if (progress.state === "error") { state.downloadInProgress = false; setText($("whisper-status"), t("downloadError")); }
        });
