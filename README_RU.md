...
### Автоматизация после распознавания
- В разделе **Automation** одно действие можно вызвать ключевым словом или `Fn + Control` на macOS.
- Ключевое слово ищется только в начале, в любом месте или только в конце; в действие отправляется остаток фразы, а не сама команда.
- Действие — прямой запуск программы с аргументами или запись текста в файл. Текст всегда приходит в stdin; shell не используется.
- Webhook настраивается обычным `curl`. Подстановки аргументов: `{text}`, `{text_json}`, `{text_url}`; необязательный секрет доступен только дочерней команде как `VOICEPASTE_ACTION_SECRET`.

### Обратная связь
- Оверлей у курсора или **по центру экрана** (toggle).
- VAD-предпросмотр: каждая законченная фраза расшифровывается один раз,
  добавляется в черновик и копируется в буфер без преждевременной вставки.
- После отпускания клавиши весь исходный WAV проходит отдельную полную
  расшифровку; только её результат вставляется в приложение с исходным фокусом.
...
VAD: чувствительность/пауза [окно Settings]
Autostart                 (toggle)
─────────────
